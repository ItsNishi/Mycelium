...
pub(crate) fn detect_hooks(pid: u32) -> Result<Vec<HookInfo>> {
    ...
    let handle = SafeHandle::open_process(
...
fn enumerate_modules(handle: HANDLE) -> Result<Vec<ModuleInfo>> {
    ...
    for hmod in &hmodules {
        ...
        let name_len = unsafe {
            GetModuleFileNameExW(Some(handle), Some(*hmod), &mut name_buf)
        } as usize;
...
fn parse_pe_exports(data: &[u8]) -> Option<PeExports> {
    ...
    for i in 0..num_names {
        ...
        let name = String::from_utf8_lossy(&data[name_file_off..name_file_off + name_end])
            .to_string();
...
fn scan_iat_hooks(
    ...
    let dos_header = read_process_memory_safe(handle, base, 0x40).ok_or_else(|| {
    ...
    for i in 0..max_import_descriptors {
        ...
        let iat_data = match read_process_memory_safe(handle, iat_addr, max_thunks * thunk_entry_size) {
            Some(d) => d,
            None => continue,
        };
        ...
        for j in 0..max_thunks {
            ...
            let in_expected = resolved_addr >= expected_module.base_address
                && resolved_addr
                    < expected_module.base_address + expected_module.size as u64;
